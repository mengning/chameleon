...
#include <sys/shm.h>
#define PORT  8989
char* sent_to_server(const u8* mac)
{ 
	char c[200];
	int  port = PORT;
 ...
 write(sock_client,c,strlen(c));
				  
 /*****receive the response from web server*****/
 static char resp[1024];
 //when the function is over,this string is still in using
 int num = 0;
...
//wanglei
	...
	{
		//broadcast,checkmac
		char* info=sent_to_server(mgmt->sa);
		int info_len = strlen(info);
		if(info_len<=3)
		printf("no matched user!\n");
		else 
         	printf("info is %s\n",info);
		//char *ssid=info;
